...
4. `simplus-agent` 继续独占 ML307A AT/APDU 端点。只有 root `simplus-netd` 及其同一 cgroup 内的 strongSwan 插件能访问单独的 mode `0600` SIM AKA socket；管理 HTTP API 永不返回 IMSI、IMPI/IMPU、AKA challenge/keys、鉴权头或节点凭据。
...
7. Web 只显示 `stopped/starting/connecting/registering/online/reconnecting/stopping/failed`、当前出口、最近上线时间、下次刷新时间和稳定错误码。不得显示 PID、SPI、IPsec key、内层地址、P-CSCF、原始运营商身份或内部网络命令。
