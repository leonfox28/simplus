...
- 线路主表在桌面使用 Ant Design Table、手机使用记录卡片，展示名称、实时模组型号/USB Serial、脱敏 SIM/Profile、运行态手机号、状态、VoWiFi 与出口，不重复罗列能力标签；响应式配置抽屉分别保存名称、出口和 VoWiFi 意图。新 Line 出口固定为 `unconfigured`，只有显式 `direct` 或 `mihomo-country` 可进入激活准入；
...
3. 在具备合适测试号码时补充 Host VoWiFi 短信其他收件人互通，并继续跟踪前端依赖审计和发布材料许可证；
