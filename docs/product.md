...
- 配置企业微信与飞书 Webhook 出站通知、测试投递及错误状态；飞书还可通过短期验证
  链接一键创建最小权限应用并绑定授权用户私聊；
- 飞书绑定不发现或选择群聊，不订阅入站事件，也不接受远程控制命令；Webhook、应用
  credential 与授权用户标识均不在普通 API、Web、SSE 或错误中回显；
- 删除飞书应用渠道只移除 Simplus 本地绑定，飞书侧应用由管理员另行处理。
