...
ALTER TABLE sms_messages_v5 RENAME TO sms_messages;
...
SELECT message_id, operation_id, direction, line_id, remote_address, body, status,
       CASE WHEN direction = 'outbound' AND status IN ('unconfirmed', 'failed') THEN '' ELSE provider_message_id END,
       error_code, created_at_unix_ms, updated_at_unix_ms, sent_at_unix_ms
FROM sms_messages;
