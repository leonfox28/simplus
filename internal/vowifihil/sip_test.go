...
func TestBuildAndParseIMSInitialRegistration(t *testing.T) {
	...
	for _, required := range []string{
		"REGISTER sip:" + testIMSHomeDomain + " SIP/2.0\r\n",
		"CSeq: 1 REGISTER\r\n", "expires=1800\r\n",
		"Authorization: Digest username=\"" + privateIdentity + "\", realm=\"" + testIMSHomeDomain + "\", nonce=\"\"",
	...
	response := "SIP/2.0 401 Unauthorized\r\n" +
		...
		"CSeq: 1 REGISTER\r\n" +
		"WWW-Authenticate: Digest realm=\"" + testIMSHomeDomain + "\", nonce=\"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\", algorithm=AKAv1-MD5, qop=\"auth\"\r\n" +
		...
		"Content-Length: 0\r\n\r\n"
	summary, err := ParseIMSInitialResponse([]byte(response), input.CallID, input.HomeDomain)
...
func TestBuildIMSInitialRegistrationAcceptsAnotherHomeNetwork(t *testing.T) {
	privateIdentity := "310260123456789@" + alternateIMSHomeDomain
	input := IMSInitialRegisterInput{
		Source: netip.MustParseAddr("10.255.0.42"), UnprotectedPort: 42000,
		ProtectedClientPort: 42001, ProtectedServerPort: 42002,
		ClientSPI: 1234567890, ServerSPI: 2234567890,
		PrivateIdentity: privateIdentity, PublicIdentity: "sip:" + privateIdentity, HomeDomain: alternateIMSHomeDomain,
		Branch: "0123456789abcdef", FromTag: "1123456789abcdef",
		CallID: "2123456789abcdef@10.255.0.42", ContactUser: "3123456789abcdef",
		WLANNodeID: "020000000001",
	}
	request, _, err := BuildIMSInitialRegister(input)
	if err != nil || !strings.Contains(string(request), "REGISTER sip:"+alternateIMSHomeDomain+" SIP/2.0") {
...
func TestParseIMSInitialResponseRejectsMissingSecurityAndWrongTransaction(t *testing.T) {
	...
	if _, err := ParseIMSInitialResponse([]byte(response), callID, testIMSHomeDomain); err == nil {
		t.Fatal("accepted challenge without Security-Server")
	}
	if _, err := ParseIMSInitialResponse([]byte(response), "3123456789abcdef@10.255.0.42", testIMSHomeDomain); err == nil {
