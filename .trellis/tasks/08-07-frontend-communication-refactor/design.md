...
- 已进入产品界面的不可用能力仍显示入口和禁用原因，不能因为后端未装配就让功能从导航消失。eUICC 是用户批准的暂缓例外：模组页在功能完整实现前不渲染其标签、提示、Profile 或操作，也不挂载 eUICC query；生成契约、后端能力与 realtime topic 保留，供后续一次性恢复完整功能。
