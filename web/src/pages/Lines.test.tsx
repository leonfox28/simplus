...
describe('Lines readiness workflows', () => {
  ...
  it('shows candidate reasons and keeps VoWiFi activation fail-closed for an unconfigured exit', async () => {
    vi.stubGlobal('fetch', vi.fn(async (request: Request) => {
      ...
      if (url.pathname === '/api/v1/vowifi-lines') return json({ lines: [{ lineId: testLine.id, desiredActive: false, eligible: false, readinessCode: 'EGRESS_NOT_CONFIGURED', state: 'stopped', stage: '', online: false, egressMode: 'unconfigured', countryCode: '', countryName: '', registeredAt: '', nextRefreshAt: '', attempt: 0, lastErrorCode: '' }] })
    ...
    expect(screen.getByText('蜂窝 SIM')).toBeInTheDocument()
  ...
  it('renders every differently sourced Line number without consulting VoWiFi identity state', async () => {
    vi.stubGlobal('fetch', vi.fn(async (request: Request) => {
      const path = new URL(request.url).pathname
      if (path === '/api/v1/lines') return json({ lines: [{ ...testLine, phoneNumbers: [
    ...
    renderPage(<Lines />)
    expect(await screen.findByText('+12025550123')).toBeInTheDocument()
